...
import 'custom_stream_audio_recorder_controller.dart';
import 'whatsapp_style_voice_recorder.dart';
...
class CustomStreamMessageInput extends StatefulWidget {
  ...
  /// Background color for the footer container (behind the input)
...
class StreamMessageInputState extends State<CustomStreamMessageInput>
    with RestorationMixin<CustomStreamMessageInput> {
  ...
  late final _audioRecorderController = CustomStreamAudioRecorderController();
  bool _isCancelWidgetVisible = false;
  ...
  Widget build(BuildContext context) {
    ...
    final bgColor = widget.backgroundColor ?? _messageInputTheme.inputBackgroundColor;
    return Material(
      color: bgColor,
  ...
  Widget _buildTextField(BuildContext context) {
    return ValueListenableBuilder(
      ...
      builder: (context, state, _) {
        ...
        // Hide buttons when recording is active OR when cancel widget is visible
        final shouldHideInputButtons = isAudioRecordingFlowActive || _isCancelWidgetVisible;

        return Directionality(
          textDirection: TextDirection.ltr,
          child: Row(
            children: [
              if (!shouldHideInputButtons) ...[
                if (!_commandEnabled &&
              ...
              if (widget.enableVoiceRecording)
                Expanded(
                  ...
                  flex: shouldHideInputButtons ? 1 : 0,
                  child: WhatsAppStyleVoiceRecorder(
                    recordState: state,
                    ...
                    onCancelWidgetVisibilityChanged: (isVisible) {
                      setState(() {
                        _isCancelWidgetVisible = isVisible;
                      });
                    },
                    onRecordStartCancel: () {
  ...
  InputDecoration _getInputDecoration(BuildContext context) {
    ...
    return InputDecoration(
      ...
      border: OutlineInputBorder(
        borderRadius: _messageInputTheme.borderRadius ?? BorderRadius.circular(8),
